...
function Splash() {
  ...
  return (
    <main className="relative flex h-dvh flex-col overflow-hidden" style={{ backgroundColor: "#0A2F7A" }}>
      <AuthOceanBackground fishCount={8} />
      ...
      <div className="relative z-10 mx-auto flex h-full w-full max-w-6xl flex-row items-center gap-6 px-5 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(1.25rem,env(safe-area-inset-top))] sm:gap-10 sm:px-8 lg:gap-14">
        ...
        >
          <img
            src={logo}
            alt="Ocean Wonders"
            className="w-[min(42vw,380px)] max-h-[70dvh] object-contain drop-shadow-[0_12px_28px_rgba(0,0,0,0.4)]"
            draggable={false}
          />
        </motion.div>
        ...
        >
          <GlossyButton variant="orange" to={playPath}>
            Play Game
          </GlossyButton>
          <GlossyButton variant="blue" to="/sea-life">
            More About Sea Life
          </GlossyButton>
          <GlossyButton variant="blue" to="/shop">
            Shop
          </GlossyButton>
        </motion.div>
      </div>
